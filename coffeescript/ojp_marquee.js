...
OJP.Marquee = Marquee = (function() {
  ...
  Marquee.prototype.SIZES = [1000, 1324, 1680];
  ...
  function Marquee(container) {
    ...
    this.min_width = this.$container.attr("data-min-width");
  ...
  Marquee.prototype.source_images = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      img = _ref[_i];
      new_source = "" + ($(img).attr('data-path')) + this.cur_img_width + "/" + ($(img).attr('data-filename'));
  ...
  Marquee.prototype.get_height = function() {
    ...
    if (this.cur_width >= this.max_width) {
      height = Math.floor(this.max_width * this.ratio) - 6;
    } else if (this.cur_width <= this.min_width) {
      height = Math.floor(this.min_width * this.ratio);
    } else {
      height = Math.floor(this.cur_width * this.ratio) - 6;
    }
    return height;
  };
  ...
  Marquee.prototype.swipe_left = function() {
    this.stop();
    return this.advance();
  };

  Marquee.prototype.swipe_right = function() {
    this.stop();
    return this.retreat();
  };
